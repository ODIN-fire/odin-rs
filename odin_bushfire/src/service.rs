...
use crate::{BushfireStore, CACHE_DIR, actor::{BushfireActorMsg, ExecSnapshotAction}, load_asset};


pub struct BushfireService {
    hactor: ActorHandle<BushfireActorMsg>
}

impl BushfireService {
    pub fn new (hactor: ActorHandle<BushfireActorMsg>)->Self {
        BushfireService{hactor}
    }
...
#[async_trait]
impl SpaService for BushfireService {
    ...
    fn add_components (&self, spa: &mut SpaComponents) -> OdinServerResult<()>  {
        ...
        spa.add_module( asset_uri!( "odin_bushfire_config.js"));
        spa.add_module( asset_uri!( "odin_bushfire.js" ));

        let dir = Arc::new(CACHE_DIR.clone());
        spa.add_route( |router, spa_server_state| {
            router.route( &format!("/{}/bushfire-data/{{*unmatched}}", spa_server_state.name.as_str()), get({
    ...
    async fn data_available (&mut self, hself: &ActorHandle<SpaServerMsg>, has_connections: bool, sender_id: &str, data_type: &str) -> OdinServerResult<bool> {
        ...
        if self.hactor.id() == sender_id && data_type == type_name::<BushfireStore>() { // is this for us?
            if has_connections {
                let action = dyn_dataref_action!( let hself: ActorHandle<SpaServerMsg> = hself.clone() => |store: &BushfireStore| {
    ...
    async fn init_connection (&mut self, hself: &ActorHandle<SpaServerMsg>, is_data_available: bool, conn: &mut WsConnection) -> OdinServerResult<()> {
        ...
        if is_data_available {
            let action = dyn_dataref_action!{
                ...
                |store: &BushfireStore| {                              // this gets executed by the BushfireActor
                    let remote_addr = remote_addr.clone();
                    let ws_msg = store.get_json_snapshot_msg();
                    Ok( hself.try_send_msg( SendWsMsg{remote_addr,ws_msg})? )
                }
            };
            self.hactor.send_msg( ExecSnapshotAction(action)).await?; // send the action requests to the BushfireActor
        }

...
use std::{borrow::Borrow, collections::HashMap, fmt::{Debug, Write}, any, fs::File, hash::Hash, io::{BufReader, Read, Write as IOWrite}, marker::PhantomData, ops::Deref, path::{Path,PathBuf}};
...
#[async_trait]
pub trait SharedStore<T> : SharedStoreReadAccess<T> + Send + Sync
...
impl<T> SharedStoreReadAccess<T> for PersistentHashMapStore<T>
    where T: SharedStoreValueConstraints
{
    fn ref_iter<'a>(&'a self)->Box<dyn Iterator<Item=(&'a String,&'a T)> + 'a> {
...
impl<T> Debug for PersistentHashMapStore<T> 
    where T: SharedStoreValueConstraints
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!( f, "ref PersistentHashMapStore<{}>", any::type_name::<T>()) 
    }
}

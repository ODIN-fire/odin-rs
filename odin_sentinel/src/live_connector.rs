                ...
                match ws_msg {
                    WsMsg::Record { device_id, sensor_no, rec_type } => {
                        ...
                        match rec_type {
                            ...
                            Detection     => Self::get_and_send_update::<DetectionData>( hself, client, config, &device_id, sensor_no, latest_recs).await,
        ...
        for (uri_path, rec_id) in latest_recs.snapshot() {
            if let Some(rec_type) = str_from_last( &uri_path, '/') {
                ...
                let res = match SensorCapability::capability_of(rec_type) {
                    ...
                    Some(Detection)      => Self::get_and_send_missing::<DetectionData>( hself, client, config, &uri_path, &rec_id, latest_recs).await,

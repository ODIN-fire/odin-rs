...
#[derive(Serialize,Deserialize,Debug,Clone)]
 ...
 impl OverpassList {
    ...
    pub fn get_next_overpass_end(&self) -> Result<DateTime<Utc>> {
        let end_dates = self.get_end_dates();
        if end_dates.len() > 0 {
            let now = Utc::now();
            let mut next_date = None;
            for dt in end_dates.iter() {
                let time_delta = now-dt; // get first date that is after now
                if time_delta.num_milliseconds() < 0 { // negative bc time is in the future
                    next_date = Some(dt.clone());
                    break;
                }
            }
    ...
    pub fn get_most_recent_overpass(&self) -> Option<Overpass> {
        let mut most_recent_overpass = None;
        let now = Utc::now();
        for overpass in self.overpasses.iter() {
            if  (now.timestamp_millis() - overpass.last_date > 0) {  // in past 
                most_recent_overpass = Some(overpass.clone())
            } else { // in future
                break
            }
        }
        most_recent_overpass
    }
 }

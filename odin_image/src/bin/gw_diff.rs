...
use odin_image::{
    get_dominant_tile_size, get_grid_dim, tiled_mean_gw, open_diff_image_pair, load_checked_horizon, hsv_horizon_line, 
...
fn main()->Result<()> {
    ...
    let (g1,w1) = tiled_mean_gw( &img1, tile_width, tile_height, ARGS.fractional_tiles, mask.as_ref(), ARGS.min_valid, &valid_pixel_pred)?;
    let (g2,w2) = tiled_mean_gw( &img2, tile_width, tile_height, ARGS.fractional_tiles, mask.as_ref(), ARGS.min_valid, &valid_pixel_pred)?;
    
    let diff_g = g2 - g1;
    let diff_w = w2 - w1;
    //let diff_w = w2.rel_diff( &w1)?;

    println!("gray difference:");
    diff_g.print( 6, 2);

    println!();
    println!("white difference:");
    diff_w.print( 6, 2);

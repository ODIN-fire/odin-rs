...
use uom::si::f32::{Area, Length, Power, ThermodynamicTemperature};
use uom::si::{
    area::square_meter, f32::Time, length::meter, power::milliwatt,
...
use odin_common::s3::{S3Client, S3Object, create_s3_client, download_s3_object, get_s3_objects};
use odin_common::{datetime::full_hour, fs::remove_old_files, ranges::LinearRange};
use odin_dem::DemSource;
use odin_gdal::gdal::{DatasetOptions, GdalOpenFlags};
use odin_gdal::{Dataset, GdalValueType, Metadata, MetadataEntry}; // gdal re-exports
use odin_gdal::{
    GridPoint, find_grid_points_in_slice, get_grid_point_values, get_linear_range, nc_dataset,
    quiet_nc_dataset,
};
...
impl GoesrHotspot {
    ...
    pub fn is_good_pixel(&self) -> bool {
        (self.mask == MASK_GOOD) || (self.mask == MASK_TEMP_GOOD)
    }
    pub fn is_high_probability_pixel(&self) -> bool {
        (self.mask == MASK_HIGH_PROB) || (self.mask == MASK_TEMP_HIGH_PROB)
    }
    pub fn is_medium_probability_pixel(&self) -> bool {
        (self.mask == MASK_MED_PROB) || (self.mask == MASK_TEMP_MED_PROB)
    }
    pub fn is_low_probability_pixel(&self) -> bool {
        (self.mask == MASK_LOW_PROB) || (self.mask == MASK_TEMP_LOW_PROB)
    }
    pub fn is_temporally_filtered(&self) -> bool {
        (self.mask == MASK_TEMP_GOOD)
            || (self.mask == MASK_TEMP_HIGH_PROB)
            || (self.mask == MASK_TEMP_MED_PROB)
            || (self.mask == MASK_TEMP_LOW_PROB)
    }
}
...
impl GoesrHotspotSet {
    pub fn new(data: &GoesrData, hotspot_vec: Vec<GoesrHotspot>) -> Self {
        ...
        for h in &hotspot_vec {
            ...
            if h.is_high_probability_pixel() {
                n_high += 1;
            } else if h.is_medium_probability_pixel() {
                n_medium += 1;
            } else if h.is_low_probability_pixel() {
                n_low += 1;
            }
        }
...
pub fn parse_goesr_create_dtg(path: impl AsRef<Path>) -> Option<DateTime<Utc>> {
    ...
    filename
        .rfind("_c")
        .and_then(|idx| parse_goesr_dtg(&filename[idx + 2..]))
}
...
pub async fn get_most_recent_objects(
    client: &S3Client,
    bucket: &str,
    source: &str,
    dur: Duration,
...
pub async fn get_objects_since_last(
    client: &S3Client,
    bucket: &str,
    source: &str,
    last_obj: &S3Object,
...
pub async fn get_objects_since(
    client: &S3Client,
    bucket: &str,
    source: &str,
    last_obj: &Option<S3Object>,
    dt: DateTime<Utc>,
    now: DateTime<Utc>,
) -> Result<Vec<S3Object>> {
...
pub async fn download_and_read_objects(
    client: &S3Client,
    bucket: &str,
    source: &Arc<String>,
    sat_id: u32,
...
pub fn read_goesr_data(data: &GoesrData) -> Result<GoesrHotspotSet> {
    ...
    let area: Vec<f32> =
        get_grid_point_values(&quiet_nc_dataset(&data.file, "Area")?, 1, NAN, &hs)?;
    let power: Vec<f32> =
        get_grid_point_values(&quiet_nc_dataset(&data.file, "Power")?, 1, NAN, &hs)?;
    let temp: Vec<f32> =
        get_grid_point_values(&quiet_nc_dataset(&data.file, "Temp")?, 1, NAN, &hs)?;
    let dqf: Vec<u8> =
        get_grid_point_values(&quiet_nc_dataset(&data.file, "DQF")?, 1, DQF_UNKNOWN, &hs)?;
